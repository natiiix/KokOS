...
BOOL read(HBA_PORT *port, QWORD start, DWORD count, WORD *buf)
{
	...
	{
		cmdtbl->prdt_entry[i].dba = (DWORD)virttophysptr(buf);
	...
	// Last entry
	cmdtbl->prdt_entry[cmdheader->prdtl-1].dba = (DWORD)virttophysptr(buf);
	...
	// Setup command
	FIS_REG_H2D *cmdfis = (FIS_REG_H2D*)phystovirtptr(&cmdtbl->cfis);
